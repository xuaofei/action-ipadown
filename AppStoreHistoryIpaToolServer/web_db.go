...
func (d *Database) UpdateTaskCompleted(taskID string, completed int) error {
	log.Printf("UpdateTaskCompleted Taskid:%v, completed:%v", taskID, completed)

	updateSQL := `UPDATE tasks SET completed = ? WHERE task_id = ?;`

	_, err := d.db.Exec(updateSQL, completed, taskID)
	if err != nil {
		log.Printf("UpdateTaskCompleted db failed:%v", err)
		return err
	}

	log.Printf("UpdateTaskCompleted inserted successfully")
	return nil
}
